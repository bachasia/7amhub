  ...
  const walk = (node: any) => {
    for (const ch of node.childNodes || []) {
      ...
      if (tag === "br") buf += "\n";
        ...
        if (u && !seenImg.has(u)) { seenImg.add(u); blocks.push({ t: "img", v: u }); }
      } else if (SKIP_TAGS.has(tag)) { /* skip */ }
      else if (BLOCK_TAGS.has(tag)) { flush(); walk(ch); flush(); }
      else { walk(ch); }
    }
  };
